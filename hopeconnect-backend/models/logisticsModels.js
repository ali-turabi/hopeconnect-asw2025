...
const getLogisticsTrackingInfo = async (logisticsId) => {
...
const createLogisticsRequest = async ({ donation_id, assigned_id }) => {
...
async function getAllMappingDonations() {
  const [rows] = await db.query('SELECT * FROM logistics_requests');
  return rows;
}

const updateLogisticsStatus = async ({ id, status, current_location, signature }) => {
  let query = `
