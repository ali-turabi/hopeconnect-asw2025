...
router.post('/mappingDonations', async (req, res) => {
  const { donation_id, assigned_id} = req.body;
   if (!donation_id || !assigned_id ) {
    return res.status(400).json({ error: 'donation_id, assigned_id, and orphanage_id are required' });
  }
...
router.get('/mappingDonations', async (req, res) => {
 try {
...
router.put('/updateMappingDonations', async (req, res) => {
...
router.delete('/mappingDonations/:id', async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await deleteLogisticsRequest(id);
...
router.get('/mappingDonations/:id', async (req, res) => {
  const { id } = req.params;
